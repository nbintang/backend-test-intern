...
export class AuthService {
  ...
  async register(dto: RegisterDto): Promise<Merchant> {
    const existedMerchant =
      await this.merchantService.findExistingMerchantByEmail(dto.email);
  ...
  async login(
    ...
    const merchant = await this.merchantService.findExistingMerchantByEmail(
      dto.email,
    );
